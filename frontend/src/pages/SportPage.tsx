...
export default function SportPage() {
  const { data, loading } = useFetch<SportRow[]>(() => publicGet<SportRow>('sport'));
      ...
      <div className="container">
        ...
        {loading ? <span className="spinner" /> : (
          <div className="vertical-list">
            {(data || []).map((item) => (
              <Link key={item.id} to={`/sport/${item.id}`} className="vertical-list-item" style={{ color: 'inherit' }}>
                {item.picture && (
            ...
            {!data?.length && <div className="empty">Нема внесени спортски настани.</div>}
          </div>
        )}
      </div>
    </div>
