import { useListTenders, QueryStatusParameter } from "@workspace/api-client-react";
...
export function TenderListPage() {
  ...
  const { data, isLoading, error } = useListTenders({ q, year, status, page, pageSize });
    ...
    <div className="container mx-auto px-4 md:px-8 py-12">
      <div className="mb-8">
        <h1 className="text-4xl font-serif font-bold mb-4 flex items-center gap-3">
          <Briefcase className="h-8 w-8 text-primary" /> Огласи
        </h1>
        <p className="text-muted-foreground text-lg max-w-2xl">
          Јавни набавки, огласи за вработување и други јавни повици од општината.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {data?.items.map(item => (
              <Card key={item.id} className={`flex flex-col h-full ${item.status === 'closed' ? 'opacity-75' : ''}`}>
                <CardHeader className="pb-3 border-b border-border/50">
                  ...
                  <CardTitle className="text-xl font-serif">
                    <Link href={`/oglasi/${item.id}`} className="hover:text-primary transition-colors line-clamp-2">
                      {item.title}
                    </Link>
                  </CardTitle>
                </CardHeader>
